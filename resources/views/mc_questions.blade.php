...
<head>
    ...
    <title>Multiple Choice Bank</title>
    ...
    <style>
        :root { --bg:#0b0f1a;--panel:#0f1630;--panel-2:#121a3a;--text:#eaf4ff;--muted:#a8b8d9;--accent:#57f1ff;--accent-2:#ff64f2;--line:rgba(255,255,255,0.12); }
        *{box-sizing:border-box;}html,body{height:100%;}
        body{margin:0;color:var(--text);font-family:Montserrat,system-ui,Segoe UI,Roboto,Ubuntu,Arial,sans-serif;background:radial-gradient(1000px 480px at 110% -10%,rgba(255,100,242,.15),transparent 60%),radial-gradient(900px 420px at -10% 110%,rgba(87,241,255,.15),transparent 60%),linear-gradient(135deg,var(--bg),#0a0f1c);}
        .wrap{max-width:1400px;margin:0 auto;padding:clamp(16px,3vw,36px);}
        ...
        .field input{background:transparent;border:none;outline:none;color:var(--text);width:240px;}
        ...
        .answers-hidden td[data-col=correct]{filter:blur(7px);}
        ...
        .flag-used{color:#4CFF96;font-weight:700;}
        @media (max-width:900px){thead th:nth-child(1),tbody td:nth-child(1){width:70px;}}
    </style>
</head>
<body>
<div class="wrap">
    <header>
        <div class="title">Multiple Choice Bank</div>
        <div class="actions">
            <label class="field" title="Search questions or options">
            ...
            <label class="switch" title="Hide correct answer">
                <input id="toggleCorrect" type="checkbox" />
                <span>Hide correct</span>
            </label>
            <span class="muted">Total: {{ isset($questions) ? $questions->count() : 0 }}</span>
            <form method="POST" action="{{ route('mcq.reset') }}" onsubmit="return confirm('Reset used status for all MC questions?');">
                @csrf
    ...
    @if (session('status'))
        <div style="margin-bottom:12px;padding:10px 12px;border:1px solid var(--line);border-radius:10px;background:rgba(87,241,255,.08);color:var(--text);">{{ session('status') }}</div>
    @endif
        ...
        <table id="mcTable">
            ...
            @forelse($questions ?? [] as $q)
                <tr>
                    ...
                    <td><div class="q">{{ $q->question }}</div></td>
                    <td>{{ $q->option_a }}</td>
                    <td>{{ $q->option_b }}</td>
                    <td>{{ $q->option_c }}</td>
                    <td data-col="correct">{{ strtoupper($q->correct) }}</td>
                    <td>{!! $q->used ? '<span class="flag-used">✓</span>' : '' !!}</td>
                </tr>
            @empty
                <tr><td class="empty" colspan="7">No multiple choice questions.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
...
const toggle = document.getElementById('toggleCorrect');
...
search?.addEventListener('input', () => {
  ...
  for (const row of tbody.rows){
    const text = Array.from(row.cells).map(c=>norm(c.innerText)).join(' ');
