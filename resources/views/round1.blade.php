...
<head>
    ...
    <style>
        ...
        * {
        ...
        .team-block.red {
            background: linear-gradient(140deg, rgba(255, 40, 40, 0.20), rgba(120, 0, 0, 0.18));
            border: 1px solid rgba(255, 80, 80, 0.35);
        }

        .team-block.blue {
            background: linear-gradient(140deg, rgba(40, 120, 255, 0.20), rgba(0, 40, 120, 0.18));
        ...
        .team-block.red .team-label {
            color: #ff6363;
        }

        .team-block.blue .team-label {
            color: #63b6ff;
        }
        ...
        .roster-list input:focus {
            border-color: #57f1ff;
            box-shadow: 0 0 0 1px #57f1ff, 0 4px 14px -4px rgba(87, 241, 255, 0.5);
            background: rgba(255, 255, 255, 0.12);
        }

        .collapsed .roster-list {
            display: none;
        }

        .collapsed .roster-groups {
            display: none;
        }
...
<body>
        ...
        <section class="stage" aria-live="polite">
            <header>
                <div class="brand">
                    ...
                    <div>
                        ...
                        <div class="badge" aria-label="Game Show Mode">NA vs EU Gameshow</div>
                    </div>
                </div>
                <div class="timer" aria-label="Countdown timer">
                    <div class="timer-row">
                        ...
                        <button class="btn secondary icon-btn" id="r1_setTimerBtn" type="button" aria-label="Set timer"
                            title="Set">
                            Set
                        </button>
                    </div>
                    <div class="timer-row">
                        <button class="btn secondary icon-btn" id="r1_pauseBtn" type="button" aria-label="Pause"
                            title="Pause">⏸</button>
                        <button class="btn icon-btn" id="r1_startBtn" type="button" aria-label="Start"
                            title="Start">▶</button>
                        <button class="btn secondary icon-btn" id="r1_resetBtn" type="button" aria-label="Reset"
                            title="Reset">↺</button>
            ...
            <div class="question-wrap" id="questionWrap" role="button" tabindex="0" aria-expanded="false"
                ...
                <div class="question-cover">
                    <svg class="reveal-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
            ...
            <footer>
                <div class="tip">YOU CAN JOIN! Check the stage VC chat for an explanation for the current round!
                </div>
                <div class="controls">
                    <a class="btn" id="nextBtn" href="{{ route('round1') }}"
                        aria-label="Next question">Next</a>
                </div>
            </footer>
        </section>
    </div>
    ...
    <script>
        ...
        function saveRoster(slot, value) {
            try {
                localStorage.setItem(rosterKeyPrefix + slot, value);
            } catch (e) {}
        }
        ...
        function r1_render() {
            r1_timeEl.textContent = r1_formatTime(r1_totalSeconds);
        }
        ...
        function r1_computeRemaining() {
            if (!r1_endTime) return r1_totalSeconds;
            const diff = Math.ceil((r1_endTime - Date.now()) / 1000);
        ...
        function r1_updateFromEnd() {
            ...
            if (remain !== r1_lastDisplayed) {
                r1_totalSeconds = remain;
                r1_render();
                r1_lastDisplayed = remain;
            } else {
                r1_totalSeconds = remain;
            }
            if (r1_endTime && remain === 0) {
                r1_stop();
                r1_clearPersist();
            }
        }
        ...
        function r1_start() {
            if (r1_timerId || r1_totalSeconds <= 0) return;
            r1_timerId = setInterval(r1_tick, 250);
            r1_tick();
        }

        function r1_stop() {
            if (r1_timerId) {
                clearInterval(r1_timerId);
                r1_timerId = null;
            }
        }

        function r1_clearPersist() {
            localStorage.removeItem(R1_LS_END);
            localStorage.removeItem(R1_LS_REM);
            r1_endTime = null;
        }
        ...
        function r1_setNew(seconds) {
            ...
            r1_totalSeconds = seconds;
            r1_render();
            localStorage.setItem(R1_LS_REM, String(r1_totalSeconds));
        }

        r1_setBtn?.addEventListener('click', () => {
            const v = r1_parse(r1_timeInput.value);
            if (v == null) {
        ...
        r1_startBtn?.addEventListener('click', () => {
            if (!r1_endTime && r1_totalSeconds === 0 && r1_timeInput.value) {
                const v = r1_parse(r1_timeInput.value);
                if (v != null) r1_setNew(v);
            }
            if (r1_totalSeconds > 0 && !r1_endTime) {
                r1_endTime = Date.now() + r1_totalSeconds * 1000;
                localStorage.setItem(R1_LS_END, String(r1_endTime));
                localStorage.removeItem(R1_LS_REM);
                r1_updateFromEnd();
                r1_start();
            }
        });
        r1_pauseBtn?.addEventListener('click', () => {
            if (r1_endTime) {
                r1_updateFromEnd();
                r1_stop();
                localStorage.setItem(R1_LS_REM, String(r1_totalSeconds));
    ...
    <aside id="rosterPanel" class="roster-panel collapsed" aria-label="Roster name slots" aria-expanded="false">
        <div class="roster-header">
            ...
            <button id="rosterToggle" type="button" class="roster-toggle" aria-pressed="false"
                aria-label="Toggle roster panel">⇔</button>
        </div>
        <div class="roster-groups">
            <div class="team-block red" aria-labelledby="red-team-label">
                <div id="red-team-label" class="team-label"><span class="dot" aria-hidden="true"></span> EU TEAM
                </div>
                <div class="roster-list" role="list">
                    @for ($i = 1; $i <= 5; $i++)
                        <input data-roster-slot="{{ $i }}" maxlength="28"
                            placeholder="Red {{ $i }}" aria-label="Red Team slot {{ $i }}" />
                    @endfor
                </div>
            </div>
            <div class="team-block blue" aria-labelledby="blue-team-label">
                <div id="blue-team-label" class="team-label"><span class="dot" aria-hidden="true"></span> NA TEAM
                </div>
                <div class="roster-list" role="list">
                    @for ($i = 6; $i <= 10; $i++)
                        <input data-roster-slot="{{ $i }}" maxlength="28"
                            placeholder="Blue {{ $i - 5 }}" aria-label="Blue Team slot {{ $i - 5 }}" />
                    @endfor
                </div>
            </div>
        </div>
    ...
    <div
        style="position:absolute;bottom:12px;left:50%;transform:translateX(-50%);z-index:5;font-family:Montserrat,system-ui,sans-serif;">
